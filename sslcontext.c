...
SSLContextCertFileInit(NsOpenSSLContext *sslcontext)
{
    ...
    if (sslcontext->keyFile == NULL || sslcontext->certFile == NULL) {
        ...
        Ns_Log(Notice,
	       "%s (%s): no cert or key defined for client SSL context '%s' (this may be ok)"
	       MODULE, sslcontext->server, sslcontext->name);
        return NS_OK;
    }
...
SSLContextProtocolsInit(NsOpenSSLContext *sslcontext)
{
...
#endif
    ;
    const struct protocol_s {
	char *code, *name; int bit;
    } protocols[] = {
#ifdef SSL_OP_NO_SSLv2
      { "sslv2", "SSLv2", SSL_OP_NO_SSLv2 },
#endif
#ifdef SSL_OP_NO_SSLv3
      { "sslv3", "SSLv3", SSL_OP_NO_SSLv3 },
#endif
#ifdef SSL_OP_NO_TLSv1
      { "tlsv1", "TLSv1", SSL_OP_NO_TLSv1 },
#endif
#ifdef SSL_OP_NO_TLSv1_1
      { "tlsv1.1", "TLSv1.1", SSL_OP_NO_TLSv1_1 },
#endif
#ifdef SSL_OP_NO_TLSv1_2
      { "tlsv1.2", "TLSv1.2", SSL_OP_NO_TLSv1_2 },
#endif
#ifdef SSL_OP_NO_TLSv1_3
      { "tlsv1.3", "TLSv1.3", SSL_OP_NO_TLSv1_3 },
#endif
    };
    ...
    if (sslcontext->protocols == NULL) {
        Ns_Log(Notice, "%s (%s): '%s' protocol parameter not set; using all protocols: SSLv2, SSLv3 and TLSv1",
                MODULE, sslcontext->server, sslcontext->name);
        bprotocols &= ~bprotocols;
    } else {
        lprotocols = ns_strdup(sslcontext->protocols);
        lprotocols = Ns_StrToLower(lprotocols);
        if (strstr(lprotocols, "all") != NULL) {
            Ns_Log(Notice, "%s (%s): '%s' using any supported protocols which may include SSLv2, SSLv3, TLSv1, TLSv1.1, and TLSv1.2",
                    MODULE, sslcontext->server, sslcontext->name);
            bprotocols &= ~bprotocols;
        }
#	if defined(SSL_OP_NO_SSLv2) || defined(SSL_OP_NO_SSLv3) || defined(SSL_OP_NO_TLSv1) || defined(SSL_OP_NO_TLSv1_1) || defined(SSL_OP_NO_TLSv1_2) || defined(SSL_OP_NO_TLSv1_3)
	  else {
	    char *where = NULL;
	    int i;
	    for (i = 0; i < sizeof(protocols) / sizeof(struct protocol_s); i++) {
		if ((where = strstr(lprotocols, protocols[i].code)) != NULL) {
		    if (where == lprotocols || (where > lprotocols && where[-1] != '-')) {
			Ns_Log(Notice, "%s (%s): '%s' using %s protocol", MODULE, sslcontext->server, sslcontext->name, protocols[i].name);
			bprotocols &= ~protocols[i].bit;
		    } else {
			Ns_Log(Notice, "%s (%s): '%s' BLOCKING %s protocol", MODULE, sslcontext->server, sslcontext->name, protocols[i].name);
			bprotocols |= protocols[i].bit;
		    }
		}
	    }
        }
#	endif /* any SSL_OP_NO_* */
        ns_free(lprotocols);
    }
    if (SSL_CTX_set_options(sslcontext->sslctx, bprotocols) == 0) {
